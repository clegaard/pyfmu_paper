...
class VirtualRobotti(VirtualFMU):
    def __init__(self, instanceName):
        ...
        self.model = RobottiDynamicModel()
        self.start_time = 0.0

        self.model.deltaFl = lambda: self.state[self.steering]
        self.model.deltaFr = lambda: self.state[self.steering]
    ...
    def reset(self):
        ...
        self.state[self.Y] = 0.0
        self.state[self.vx] = 1.0
        self.model.set_time(self.start_time)
    ...
    def doStep(self, currentCommunicationPoint, communicationStepSize, noSetFMUStatePriorToCurrentPoint=fmi2True):
        ...
        x = self.model.state_vector()
        assert np.isclose(self.model.time(), currentCommunicationPoint)
        ...
        assert sol.success
        assert np.isclose(self.model.time(), currentCommunicationPoint + communicationStepSize)

        self.state[self.X] = self.model.X()
        self.state[self.Y] = self.model.Y()
        self.state[self.vx] = self.model.vx()
