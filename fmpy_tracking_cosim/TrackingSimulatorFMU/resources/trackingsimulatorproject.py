...
class TrackingSimulatorProject(Fmi2Slave):
    ...
    def do_step(self, current_time: float, step_size: float, no_set_fmu_state_prior: bool) -> bool:
        try:
            ...
            if()
            assert np.isclose(
                self.model.time(), current_time), "np.isclose(self.model.time(), current_time)"
            assert np.isclose(self.model.dbike.time(
            ), current_time), "np.isclose(self.model.dbike.time(), current_time)"
            sol = solve_ivp(f, (current_time, current_time+step_size), x, method=StepRK45,
                            max_step=step_size, model=self.model, t_eval=[current_time+step_size])
            assert sol.success
            assert np.isclose(self.model.time(
            ), current_time + step_size), "np.isclose(self.model.time(), current_time + step_size)"
            assert np.isclose(self.model.dbike.time(
            ), current_time + step_size), "np.isclose(self.model.dbike.time(), current_time + step_size)"
...
class BikeDynamicModelSpeedDriven(Model):
    def __init__(self):
        ...
        self.deltaf = self.input(lambda: 0.0)
        self.af = self.var(self.get_af)  # Front Tire slip angle";
        # Rear Tire slip angle";
        self.ar = self.var(lambda: (self.vy() - self.lr*self.dpsi())/self.vx())
        # lateral tire force at the front tire in the frame of the front tire";
        self.Fcf = self.var(lambda: self.Caf()*self.af())
        # lateral tire force at the rear tire in the frame of the rear tire";
        self.Fcr = self.var(lambda: self.Car*(-self.ar()))

        self.der('x', lambda: self.vx())
        self.der('y', lambda: self.vy())
        self.der('psi', lambda: self.dpsi())
        self.der('vy', lambda: -self.dpsi()*self.vx() + (1/self.m) *
                 (self.Fcf() * math.cos(self.deltaf()) + self.Fcr()))
        self.der('dpsi', lambda: (2/self.Iz) *
                 (self.lf*self.Fcf() - self.lr*self.Fcr()))
        self.der('X', lambda: self.vx()*math.cos(self.psi()) -
                 self.vy()*math.sin(self.psi()))
        self.der('Y', lambda: self.vx()*math.sin(self.psi()) +
                 self.vy()*math.cos(self.psi()))
