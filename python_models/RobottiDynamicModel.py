...
class RobottiDynamicModel(Model):
    def __init__(self):
        ...
        self.Car = self.parameter(38323.06343)  # Rear Tire cornering stiffness";
        ...
        self.Nlfr = self.parameter(self.m_robot_r*self.g*(self.lr/self.L)+self.m_ext_r*self.g)  # Normal load on the rear right
        ...
        self.Caf = self.input(lambda: 38323.06343)  # Front Tire cornering stiffness";
        ...
        self.vel_left = self.input(lambda: 0.0)  # speed of the left wheel
        self.vel_right = self.input(lambda: 0.0)  # speed of the right wheel
        ...
        self.FyflT = self.var(lambda: -self.mu*self.Nlfr*sign(self.alphaFr())*(1-(self.mu*self.Nlfr) / (4*self.Caf()*abs(math.tan(self.alphaFr())))) if self.useNLFLT() > 0 else -self.Caf()*math.tan(self.alphaFr()))
        ...
        # Back left tyre non linear model
        self.useNLBLT = self.var(lambda: abs(-self.Car*math.tan(self.alphaRr())) - self.mu*self.Nlrr/2)
        self.FyrrT = self.var(lambda: -self.mu*self.Nlrr*sign(self.alphaRr())*(1-(self.mu*self.Nlrr) / (4*self.Car*abs(math.tan(self.alphaRr())))) if self.useNLBLT() > 0 else -self.Car*math.tan(self.alphaRr()))
